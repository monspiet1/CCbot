...
template_ementa = PromptTemplate.from_template(
    """
    Você é um assistente que ajuda pessoas a entenderem sobre a ementa de algumas matérias.
...
def rag_ementa(user_query:str) -> str:
  context = db_ementa.similarity_search(user_query, k = 3) # traz os documentos mais semelhantes

  if len(context) == 0:
    return "Eu não sou capaz de responder esta pergunta"

  context = "\n\n".join([f"## Documento {k}\n" + doc.page_content + "\nFonte: " + doc.metadata.get("fonte", "") for k, doc in enumerate(context, start=1)])

  chain = (template_ementa |
